...
export default function UniCreditConsumerLoanCalculator({ analysisId, clientId }) {
  ...
  const validation = useMemo(() => {
    ...
    if (amountBGN > 250000) errors.push('Максимална сума: 250,000 BGN (127,824 EUR)');
    if (inputs.loanTerm < 12) errors.push('Минимален срок: 12 месеца');
    if (inputs.loanTerm > 72) errors.push('Максимален срок: 72 месеца');
  ...
  const calculations = useMemo(() => {
    ...
    if (amountBGN >= 20000 && amountBGN <= 100000) interestRate = 4.50;
    else if (amountBGN > 100000) interestRate = 3.90;
    
    if (inputs.loanTerm >= 36 && inputs.loanTerm <= 60) interestRate = 4.90;
    else if (inputs.loanTerm > 60) interestRate = 3.90;
    
    if (inputs.salaryTransfer) interestRate -= 0.4;
    
    interestRate = Math.max(interestRate, 3.5);
    ...
    // Такси
    const processingFee = Math.min(Math.max(inputs.loanAmount * 0.01, 20 / EUR_BGN_RATE), 500 / EUR_BGN_RATE);
    const monthlyManagementFee = 2.00;
    ...
    <Card>
      <CardHeader className="bg-gradient-to-r from-red-50 to-red-100">
        <CardTitle className="flex items-center gap-2">
          ...
          UniCredit Булбанк - Потребителски кредит
        </CardTitle>
      </CardHeader>
      <CardContent className="p-6 space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            />
            <p className="text-xs text-slate-500 mt-1">Минимум: 511 EUR | Максимум: 127,824 EUR</p>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-slate-500 mt-1">Минимум: 12 | Максимум: 72</p>
          </div>
          ...
          <div className="flex items-center gap-2 md:col-span-2">
            <input
              ...
              className="w-4 h-4"
            />
            <Label>Превод на заплата (-0.4%)</Label>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6">
          <div className="grid md:grid-cols-3 gap-4 mb-4">
            <div>
              ...
              <p className="text-xs text-slate-500">Само погасяване</p>
            </div>
            <div>
              ...
              <p className="text-xs text-slate-500">С управление</p>
            </div>
            <div>
        ...
        <div className="bg-green-50 border-l-4 border-green-400 p-4 rounded text-sm">
          ...
          <ul className="text-green-800 space-y-1 text-xs">
            ...
            <li>✓ До 3 месеца гратисен период</li>
            <li>✓ Без изискване за кредитна история</li>
          </ul>
        </div>
      </CardContent>
    </Card>
